<script>
  ...
  function setUpEditorEvents() {
      ...
      const debouncedEmit = emitInputEvent.debounce(INPUT_EVENT_DEBOUNCE_WAIT);
      // Debounce emitting the #input event. When data is huge, instance#getData()
      // takes a lot of time to execute on every single key press and ruins the UX.
      instance.model.document.on( 'change:data', debouncedEmit
      )
    ...
    function handleBlur() {
      //console.warn("container blur")
    }
</script>

<div bind:this={editorElement} on:blur={handleBlur}></div>
